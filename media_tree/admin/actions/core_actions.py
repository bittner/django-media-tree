...
from media_tree.admin.actions.forms import (
    FileNodeActionsWithUserForm, MoveSelectedForm, CopySelectedForm,
    ChangeMetadataForSelectedForm)
...
def filenode_admin_action(modeladmin, request, queryset, form_class,
                          extra_context, success_messages, form_initial=None,
                          is_recursive=True):
    """
    """
    ...
    if execute:
        ...
        if form.is_valid():
            ...
            messages.success(request, message=ungettext(success_messages[0],
                             success_messages[1], form.success_count) % {
            ...
            # return HttpResponseRedirect(reverse('admin:media_tree_filenode_folder_expand', args=(redirect_node.pk,)))  # noqa
...
def move_selected(modeladmin, request, queryset):
    success_messages = [
        '%(count)i %(verbose_name)s moved.',
        '%(count)i %(verbose_name_plural)s moved.',
    ]
    extra_context = ({
        'title': _('Move media objects'),
        'submit_label': _('Move'),
    })
    return filenode_admin_action(
        modeladmin, request, queryset, MoveSelectedForm, extra_context,
        success_messages)
...
def copy_selected(modeladmin, request, queryset):
    success_messages = [
        '%(count)i %(verbose_name)s copied.',
        '%(count)i %(verbose_name_plural)s copied.',
    ]
    extra_context = ({
    ...
    return filenode_admin_action(
        modeladmin, request, queryset, CopySelectedForm, extra_context,
        success_messages)
...
expand_selected.short_description = \
    _('Expand selected %(verbose_name_plural)s') % {
...
collapse_selected.short_description = \
    _('Collapse selected %(verbose_name_plural)s') % {
        'verbose_name_plural': _('folders')}


def change_metadata_for_selected(modeladmin, request, queryset):
    # TODO Use AdminDateTimeWidget etc
    # TODO Should be able to leave required fields blank if confirm. not checked
    ...
    success_messages = [
        '%(count)i %(verbose_name)s changed.',
        '%(count)i %(verbose_name_plural)s changed.',
    ]
    extra_context = ({
    ...
    return filenode_admin_action(
        ...
        ChangeMetadataForSelectedForm, extra_context, success_messages,
        form_initial=initial)

change_metadata_for_selected.short_description = \
    _('Change metadata for selected %(verbose_name_plural)s')
