...
class MediaTreeGallery(MediaTreeImageListingBase):
    list_type = models.CharField(_('gallery type'), max_length=1,
        default=LISTING_MERGED,
        choices=((LISTING_MERGED, _('merged')), (LISTING_NESTED, _('nested'))),
        help_text=_('A nested gallery includes a browseable folder list. '
                    'A merged gallery displays media from all folders merged into a flat list.')
    )
